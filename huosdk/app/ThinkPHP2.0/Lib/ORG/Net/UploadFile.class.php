...
{//类定义开始
    ...
    {
        $filename = $file['f_savepath'].$file['f_savename'];
        ...
        // 如果是图像文件 检测文件格式
        if( in_array(strtolower($file['f_extension']),array('gif','jpg','jpeg','bmp','png','swf')) && false === getimagesize($file['tmp_name'])) {
            $this->error = '非法图像文件';
            return false;
        }
    ...
    public function upload($savePath ='')
    {
        ...
        $fileInfo = array();
        $isUpload   = true; //可以不选择上传文件所以设置为true
        ...
        foreach($files as $key => $file) {
            ...
            if(!empty($file['name'])) {
                //登记上传文件的扩展信息
                $file['key']          =  $key;
                $file['f_extension']  = $this->getExt($file['name']);
                $file['f_savepath']   = $savePath;
				if($this->getExt($file['name']))
                $file['f_savename']   = $this->getSaveName($file);
				$file['f_name']   = $file['name'];
				$file['f_size']   = $file['size'];
				$file['f_type']   = $file['type'];
				list($file['f_width'], $file['f_height']) = getimagesize($file['tmp_name']);
                ...
                if(function_exists($this->hashType)) {
                    $fun =  $this->hashType;
                    $file['f_hash']   =  $fun(auto_charset($file['f_savepath'].$file['f_savename'],'utf-8','gbk'));
                }
                //上传成功后保存文件信息，供其他地方调用
    ...
    private function getSaveName($filename)
    {
        ...
        if(empty($rule)) {//没有定义命名规则，则保持文件名不变
            $saveName = $filename['f_name'];
        }else {
            if(function_exists($rule)) {
                //使用函数生成一个唯一文件标识号
                $saveName = $rule().".".$filename['f_extension'];
            }else {
                //使用给定的文件名作为标识号
                $saveName = $rule.".".$filename['f_extension'];
            }
    ...
    private function check($file) {
        ...
        //检查文件类型
        if(!$this->checkExt($file['f_extension'])) {
