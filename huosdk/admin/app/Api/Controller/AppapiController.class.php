...
class AppapiController extends Controller {
    ...
    public function index(){
        $imageurl = $this->website."/upfiles/image/";//图片路径
    
        $indexDao = M('appindex');
        $indexlist = $indexDao -> field("lunbotu,onegameid,twgameid,thrgameid,fourgameid")->where(" isdelete='0' ") -> order('id DESC') -> select(); //查询游戏类型
        $imagelist = explode(",", $indexlist[0]['lunbotu']);
        	
        $arr = array();
        foreach ($imagelist as $key =>$val){
            if ($key == 0) {
                $image1['u'] = $imageurl.$val;
                $image1['g'] = $indexlist[0]['onegameid'];
            } else if ($key == 1) {
                $image2['u'] = $imageurl.$val;
                $image2['g'] = $indexlist[0]['twgameid'];
            } else if ($key == 2) {
                $image3['u'] = $imageurl.$val;
                $image3['g'] = $indexlist[0]['thrgameid'];
            } else if ($key == 3) {
                $image4['u'] = $imageurl.$val;
    ...
    public function jfcount(){
        ...
        $list=$jifens->field("jifen")->where("username='".$username."'")->limit(1)->select();
    ...
    public function jflog(){
        ...
        $jifenall=M('jifenlog');
        $list=$jifenall->field("jifen as a,create_time as b,beizhu as c")->where("username='".$username."'")->select();
        ...
        $lists=$jifenalls->field("jifen as a,create_time as b,beizhu as c")->where("username='".$username."'")->limit($offset . ',' .  $pagesize)->select();
    ...
    public function lblog(){
        ...
        $jifenall=M('ttbpay');
        $list=$jifenall->field("ttb as a,paytypeid as b,create_time as c,status as d")->where("username='".$username."'")->select();
        ...
        $offset=$pagesize*($page-1);
        $jifenalls=M('ttbpay');
        $lists=$jifenalls->field("ttb as a,paytypeid as b,create_time as c,status as d")->where("username='".$username."'")->limit($offset . ',' .  $pagesize)->select();
        	
        $type=M('paytype');
        	
        $listtype=$type->field("id as a , payname as b") -> select();
    ...
    public function reg(){
        ...
        if(!empty($username) && !empty($password)){
            ...
            $password = pw_auth_code($password);
    ...
    public function login(){
        ...
        $model = M('members');
        $userlist = $model->field("username as b")->where("username='".$username."' AND password='".$password."'")->select();
        $userlist = $userlist[0];
    ...
    public function updatePwd(){
        ...
        $newpwd = pw_auth_code($newpwd);
        $ckupdate = 0;
    
        $model = M('members');
        $userlist = $model->field("username as b")->where("username='".$username."' AND password='".$oldpwd."'")->select();
        $userlist = $userlist[0];
    ...
    public function tgameList(){
        ...
        $typeid = isset($urldata['typeid']) ? $urldata['typeid'] : 0;
        ...
        $field = "c_appgame.id as id,name,image,count,size,androidurl,c_appgame.create_time,description";
        ...
        $baoming = M('app_baoming');
        $baolist = $baoming->field("id,gameid,name")->select();
    ...
    public function gameList(){
        ...
        $field = "c_appgame.id as id,name,image,count,size,androidurl,description";
        $join = "c_appgameinfo ON c_appgame.id=c_appgameinfo.id";
    ...
    public function gameDetail(){
        ...
        $id = isset($urldata['id']) ? $urldata['id'] : 0;//获取游戏ID
    
        $game = M('appgame');
    
        $field = "c_appgame.id as id,name,image,count,type,smallimage,size,versions,androidurl,description,function";
        $join = "c_appgameinfo ON c_appgame.id=c_appgameinfo.id";
        $where = " c_appgame.isdelete=0 AND c_appgame.id=".$id;
    
        $gamelist = $game->field($field)->join($join)->where($where)->select();
        ...
        $typemodel = M('appgametype');
        $typelist = $typemodel->field("id,name")->select();
    ...
    public function serverList(){
        ...
        $game = M('appgame');
        $field = "c_appgame.id as id,name,image,androidurl";
        $join = "c_appgameinfo ON c_appgame.id=c_appgameinfo.id";
        $where = " c_appgame.isdelete=0";
        $gamelist = $game->field($field)->join($join)->where($where)->select();
        foreach ($gamelist as $key => $val) {
        ...
        foreach ($serverlist as $key=>$val) {
            ...
            if($val['sertime'] >= $todaystart && $val['sertime'] < $todayend){
                ...
                $arr2['f'] = $gamedata[$val['gameid']]['androidurl'];
            ...
            if ($hours>=0) {
                ...
                $arr1['e'] = $days."天";
                $arr1['f'] = $gamedata[$val['gameid']]['androidurl'];
                ...
                $arr3['f'] = $gamedata[$val['gameid']]['androidurl'];
    ...
    public function giftList(){
        ...
        $field = "c_applibaoinfo.id as a,c_applibaoinfo.title as b,c_applibaoinfo.content as c,CONCAT('".$this->website."/upfiles/image/',c_appgame.image) AS d,c_applibaoinfo.starttime as e";
        $join = " c_appgame ON c_applibaoinfo.gameid=c_appgame.id";
        $giftlist = $libaoinfo->field($field)->join($join)->where("c_applibaoinfo.isdelete=0 and c_applibaoinfo.endtime>'".$time."'")->order('starttime ASC')->select();
        $arr=array();
        foreach ($giftlist as $key => $val){
            $libaos = M('applibao');
            $total = $libaos->where("status=0 and infoid=".$val['a'])->count();
            $qcode=$libaos->field("code")->where("username='".$username."' and infoid=".$val['a'])->limit(1)->select();
    ...
    public function giftDetail() {
        ...
        $urldata = get_object_vars(json_decode($urldata));
        $username=isset($urldata['username']) ? $urldata['username'] : '';
        $id = isset($urldata['infoid']) ? $urldata['infoid'] : 0;//礼包 ID
        //$id = 1;
        $libaoinfo = M('applibaoinfo');
        $field = "c_applibaoinfo.id as a,c_applibaoinfo.gameid as b,c_applibaoinfo.title as c,c_applibaoinfo.content as d,CONCAT('".$this->website."/upfiles/image/',c_appgame.image) AS e,c_applibaoinfo.starttime as f,c_applibaoinfo.endtime as g";
        $join = " c_appgame ON c_applibaoinfo.gameid=c_appgame.id";
        $giftlist = $libaoinfo->field($field)->join($join)->where("c_applibaoinfo.id=".$id)->select();
        $giftlist = $giftlist[0];
		if (empty($giftlist['a'])){
			$giftlist['a'] = 0;
		}
        ...
        $qcode=$libao->field("code")->where("username='".$username."' and infoid=".$id)->limit(1)->select();
        ...
        //获取包名
        $baoming = M('app_baoming');
        $bao = $baoming->where("gameid=".$giftlist['b'])->select();
        $giftlist['i'] = $bao[0]['name'];
    
        //获取游戏下载地址
        $model = M('appgameinfo');
        $gameinfo = $model->field("androidurl")->where("id=".$giftlist['b'])->select();
        $giftlist['j'] = $gameinfo[0]['androidurl'];
    ...
    public function setLibao() {
        ...
        $infoid = isset($urldata['infoid']) ? $urldata['infoid'] : 0;//礼包 ID
        //$username = isset($urldata['username']) ? $urldata['username'] : '';//用户账号
    
        //$usermodel = M('members');
        //$userlist = $usermodel->field("id,username")->where("username='".$username."'")->select();
        ...
        $model = M('applibao');
        $list = $model->field("id,code")->where("infoid=".$infoid." and status=0")->limit(1)->select();
        $code = $list[0]['code'];
        $id = $list[0]['id'];
    ...
    public function jfList(){
        ...
        $field = "id as a,title as b,number as c,content as d,CONCAT('".$this->website."/upfiles/image/',ico) AS e,type as f,endtime as j,create_time as k";
        ...
        foreach ($wupinlist as $key => $val){
            ...
            $wupin = M('jfwupin');
            if($val['f'] == '1'){
                $clist = $wupin->field("count,quantity")->where("jfinfoid=".$val['a'])->select();
                $total =$clist[0]['count'];
    ...
    public function jfDetail() {
        ...
        $wupininfo = M('jfwupininfo');
        $field = "id as a,title as b,number as c,content as d,CONCAT('".$this->website."/upfiles/image/',ico) AS e,
				type as f,endtime as j,create_time as k";
    
        $wupinlist = $wupininfo->field($field)->where("id=".$id)->select();
        $wupinlist = $wupinlist[0];
    
        //剩余数量
        $wupin = M('jfwupin');
        if($wupinlist['f'] == '1'){
            $clist = $wupin->field("count")->where("count>0 and jfinfoid=".$wupinlist['a'])->select();
            $total =$clist[0]['count'];
        }else{
    ...
    public function setjfLibao() {
        ...
        if($str == $sign && !empty($str)){
            $usermodel = M('members');
            $jfmodel = M('jifen');
            $jifenlist = $jfmodel->field("jifen")->where("username='".$username."'")->select();//积分数
    
    
            $field = "c_jifen.jifen as jifen,c_members.username as username";
            $join = " c_jifen ON c_members.username=c_jifen.username";
            $userlist = $usermodel->field($field)->join($join)->where("c_members.username='".$username."'")->select();
            	
            $wupininfo = M('jfwupininfo');
            $infolist = $wupininfo->field("id,title,number,endtime")->where("id=".$infoid." and isdelete='0'")->select();
            ...
            if($jifen > $userlist[0]['jifen']){
                ...
                if(time() > $infolist[0]['endtime']){
                    ...
                    //返回兑换码
                    $model = M('jfwupin');
                    $list = $model->field("id,code")->where("jfinfoid=".$infoid." and status=0")->limit(1)->select();
                    if(count($list) > 0){
                        $code = $list[0]['code'];
                        $id = $list[0]['id'];
        ...
        if($str == $sign && !empty($str)){
            ...
            if($jifen > $userlist[0]['jifen']){
                ...
                if(time() > $infolist[0]['endtime']){
                    ...
                    //提交地址
                    $model = M('jfwupin');
                    $list = $model->field("id,quantity")->where("jfinfoid=".$infoid)->limit(1)->select();
    ...
    public function getMyGameList() {
        $join = "c_appgameinfo ON c_appgameinfo.id=c_app_baoming.gameid";
        $field = "gameid as a,name as b,size as c";
        $baoming = M('app_baoming');
        $baolist = $baoming->field($field)->join($join)->select();
        ...
        foreach ($baolist as $key=>$val) {
            ...
            $libaodata = $libaoinfo->field("id,endtime")->where("gameid=".$val['a']." and isdelete=0")->order("create_time DESC")->limit(1)->select();
            ...
            $serverdata = $serverinfo->field("id,sertime")->where("gameid=".$val['a']." and isdelete=0")->order("create_time DESC")->limit(1)->select();
    ...
    public function getUpdateInfo() {
        ...
        $join = "c_appgameinfo on c_appgame.id=c_appgameinfo.id";
        $field = "name,image,versions,size,androidurl";
        $arr = array();
        $data = array();
        foreach ($urldata as $key=>$val) {
            $val = get_object_vars($val);
            $baodata = $bao->field("gameid")->where("name='".$val['b']."'")->select();
            	
            if ($baodata[0]['gameid']) {
                $gamedata = $game->field($field)->join($join)->where('c_appgame.id='.$baodata[0]['gameid'])->select();
    
                if ($gamedata[0]['versions'] != $val['c']) {
                    $arr['a'] = $val['b'];
                    $arr['b'] = $gamedata[0]['versions'];
                    $arr['c'] = $gamedata[0]['androidurl'];
    ...
    public function setCount() {
        ...
        $model = M('app_baoming');
        $baoming = $model->field("gameid")->where("name='".$name."'")->select();
    
		if (!empty($baoming[0]['gameid'])){
			$model = M('appgame');
    ...
    public function getVersions() {
        ...
        $versions = isset($urldata['versions']) ? floatval($urldata['versions']) : 0;
    
        $model = M('appversion');
        $appdata = $model->field('versions,url,size,content')->where("versions>".$versions)->limit(1)->select();

        $arr = array();
        if ($appdata) {
            $old = $model->field('size')->where("versions=".$versions)->select();
    ...
    public function getKeyGame(){
        ...
        $gamelist = $game->field($field)->where($where)->order('CONVERT(NAME USING gbk)')->select();
    ...
    public function  querygiftBYname(){
        ...
        $list = $libao->field("username as a ,code as b")->where("infoid = '".$infoid."'")->select();
        foreach($list as $key=>$val){
            if($val['a']==$username){
                $msg="1";
                $arr = array(
        ...
        $list = $libao->field("id,code")->where("infoid=".$infoid." and status=0")->limit(1)->select();
    ...
    public function amoyGift(){
        ...
        $model = M('applibao');
        $datatime = time();
        $list = $model->field("code as a,update_time as b")->where("infoid='".$infoid."'")->select();
        $arr = array();
        foreach($list as $key=>$val){
    ...
    public function getFindGame(){
        ...
        $field = "c_appgame.id as id,name,image,count,size,androidurl";
        $join = "c_appgameinfo ON c_appgame.id=c_appgameinfo.id";
    
        $gamelist = $game->field($field)->join($join)->where($where)->order('CONVERT(name USING gbk)')->select();
    ...
    public function rwList(){
        ...
        $field = "c_apptask.id as a,c_apptask.title as b,c_apptask.content as c,CONCAT($this->website,'/upfiles/image/',c_appgame.image) AS d,c_apptask.jifen as e,c_apptask.gameid as f,c_apptask.endtime as h,c_apptask.starttime as i";
    ...
    public function setTask() {
        ...
        if($str == $sign && !empty($str)){
            ...
            $task = M('apptask');
            $infolist = $task->field("id,title,jifen,endtime")->where("id=".$taskid." and isdelete='0'")->select();
