    	    ...
    	    if(!empty($_POST['role_id'])){
    		    ...
    		    if ($this->users_model->create()) {
    		        ...
    		        if ($result!==false) {
	        			...
	        			$agent_game_add_data_log=M("users")->where(array('id'=>$result))->find();
	                    $agent_str='';
	                    $agent_str.=date("Y-m-d H:i:s")."->";
	                    $agent_str.="登陆名：".$agent_game_add_data_log['user_login'];
	                    $agent_str.="，昵称：".$agent_game_add_data_log['user_nicename'];
	                    $agent_str.="，联系人：".$agent_game_add_data_log['linkman'];
	                    $agent_str.="，渠道id：".$agent_game_add_data_log['id']."添加成功！\n";
