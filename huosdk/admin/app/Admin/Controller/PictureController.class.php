...
class PictureController extends AdminbaseController {
	...
	public function modifyPicture(){
		$picture = M('contact');
		$type = isset($_REQUEST['type']) ? $_REQUEST['type'] : 'logo';
 		$where = " class = 2 ";
 		$where .= " and name = '".$type."'";
		$indexdata = $picture -> where($where) -> select();
	...
	public function modifyPicture_post(){
		$indexid = I('indexid');
	 ...
	 * 上传图片
	 */
	public function uploadImage($up_info,$type,$url) {
		$arrType=array('image/jpg','image/gif','image/png','image/bmp','image/pjpeg','image/jpeg', 'image/ico');
