...
<body class="J_scroll_fixed">
	<div class="wrap js-check-wrap">
		<form class="well form-search" method="post" action="<?php echo U('Gm/office');?>">
			<div class="search_type cc mb10">
				<div class="mb10">
					...
					<select class="select_2" name="gid" id="selected_id">
				   	...
				   	账号类型
					<select class="select_2" name="flag" id="selected_id">
						<?php if(is_array($takeclasses)): foreach($takeclasses as $k=>$vo): $fid_select=$k==$formget['flag'] ?"selected":""; ?>
						<option value="<?php echo ($k); ?>"<?php echo ($fid_select); ?>><?php echo ($vo); ?></option><?php endforeach; endif; ?>
				    </select>
				    
					 &nbsp;&nbsp; 
				 	  发放账号
					 <input type="text" name="adminname"
						style="width: 200px;" value="<?php echo ($formget["adminname"]); ?>"
						placeholder="请输入发放账号..."> 
					 &nbsp;&nbsp; 
		...
		<form class="js-ajax-form" action="" method="post">
			<table class="table table-hover table-bordered table-list">
				...
				<?php if(is_array($items)): foreach($items as $key=>$vo): ?><tr>
					...
					<td><?php echo ($takeclasses[$vo['flag']]); ?></td>
