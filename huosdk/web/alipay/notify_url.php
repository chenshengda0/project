...
if($verify_result) {//验证成功
    ...
    if($_POST['trade_status'] == 'TRADE_FINISHED') {
		...
		$sql = "select order_id,mem_id,money,ptb_cnt,status,create_time from c_ptb_charge where order_id='".$out_trade_no."'";
		$rs = mysql_query($sql);
		$data = array();
		...
		if (!empty($data) && $data['status'] == 1) {
			...
			if ($data['money'] == $amount) {
				//$sql = "update c_ttbpay set status=1 where orderid='".$out_trade_no."'";
				$sql = "update c_ptb_charge set status=2 where order_id='".$out_trade_no."'";
				$rs = mysql_query($sql);
				...
				//$sql = "update c_ttbpay set status=2 where orderid='".$out_trade_no."'";
				$sql = "update c_ptb_pay set status=3 where order_id='".$out_trade_no."'";
				$rs = mysql_query($sql);
				if ($rs) {
					exit("支付失败，请重试!");
				}
