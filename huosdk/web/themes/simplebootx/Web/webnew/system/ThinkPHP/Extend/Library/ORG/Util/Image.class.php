...
class Image {
     ...
     * @return string
     */
    static function buildImageVerify($length=4, $mode=5, $type='png', $width=100, $height=30, $verifyName='verify', $fontface='Elephant.ttf') {
    	import('ORG.Util.String');
    	$randval = String::randString($length, $mode);
    	$width = ($length * 25) > $width ? $length * 25 : $width;
    	$session = new es_session();
    	$session->set($verifyName, $randval);
    	$im = imagecreatetruecolor($width, $height);
    	$borderColor = imagecolorallocate($im, 100, 100, 100);                    //边框色
    	...
    	// 干扰
    	for ($i = 0; $i < 15; $i++) {
    		$fontcolor = imagecolorallocate($im, mt_rand(120, 255), mt_rand(120, 255), mt_rand(120, 255));
    	...
    	for ($i = 0; $i < $length; $i++) {
    		...
    		$codex = $randval{$i};
    		imagettftext($im, mt_rand($width/$length/2, $width/$length/1.5), mt_rand(-30, 30), $i * ceil($width/$length)+ceil($width/$length/3), mt_rand(ceil($height/2), ceil($height/1.5)), $fontcolor, $fontface, $codex);
    	}
