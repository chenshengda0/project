...
function U($url='',$vars='',$suffix=true,$redirect=false,$domain=false) {
	...
	if(C('URL_MODEL') == 0) { // 普通模式URL转换
		...
		if(isset($route)) {
			$url    =   sysconf("ADMIN_URL").'/'.rtrim($url,$depr);
		}else{
			$url    =   sysconf("ADMIN_URL").'/'.implode($depr,array_reverse($var));
		}
		if(!empty($vars)) { // 添加参数
	...
	if(defined("ADMIN_ROOT")){
		if($suffix && substr($url,strlen($url)-strlen($suffix),strlen($suffix)) == $suffix){
...
///前台
function U2($url='',$vars='',$suffix=true,$redirect=false,$domain=false) {
    // 解析URL
    $info   =  parse_url($url);
    $url    =  !empty($info['path'])?$info['path']:ACTION_NAME;
    if(isset($info['fragment'])) { // 解析锚点
