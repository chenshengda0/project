...
class GiftController extends AdminbaseController
{
    ...
    public function index() {
        ...
        if ($show == "mygift") {
            ...
            $giftdata = M('gift')->alias('gf')->field($field)->join(
                    "LEFT JOIN " . C('DB_PREFIX') . "gift_code gfc ON gfc.gf_id=gf.id")->where($where)->order(
                    'gfc.update_time desc')->select();
            ...
            $giftdata = M('gift')->field($field)->where($where)->order("start_time ASC")->select();
            $this->assign('gifts', $giftdata);
            $this->display();
        }
    }
    public function giftAjax() {
        ...
        if (!empty($gf_id)) {
            ...
            if (0 == $cnt) {
                $app_id = $_SESSION['app_id'];
                $rs = M("gift")->where(array(
                    'id' => $gf_id, 
                    'app_id' => $app_id 
                ))->setDec('remain');
                if ($rs > 0) {
                    $field = "code, id";
                    $giftdata = $gfc_model->field($field)->where(array(
                        'gf_id' => $gf_id, 
                        'mem_id' => 0 
                    ))->find();
                    $rs = $gfc_model->where(array(
                        'id' => $giftdata['id'] 
                    ))->setField('mem_id', $mem_id);
                    if ($rs) {
                        $data['b'] = $gf_model->where(array(
                            'id' => $gf_id, 
                            'app_id' => $app_id 
                        ))->getField('remain');
