...
if(file_exists(SITE_PATH."conf/float/config.php")){
		$runtime_config=include SITE_PATH."conf/float/config.php";
}else{
    $runtime_config=array();
}

if (file_exists(SITE_PATH."conf/float/route.php")) {
    $routes = include SITE_PATH.'conf/float/route.php';
} else {
    $routes = array();
}
...
$configs= array(
    ...
    'SP_TMPL_PATH' => 'float/themes/',  // 前台模板文件根目录
    ...
    'SP_TMPL_ACTION_SUCCESS' => 'success',  // 默认成功跳转对应的模板文件,注：相对于前台模板路径
		'SP_ADMIN_STYLE'		=> 'float',
		'SP_ADMIN_TMPL_PATH'    => 'float/themes/',       // 各个项目后台模板文件根目录
		'SP_ADMIN_DEFAULT_THEME'=> 'web',       // 各个项目后台模板文件
