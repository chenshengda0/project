		...
		if ('Forgetpwd' != CONTROLLER_NAME &&
			...
			if (isset($_SESSION['user_token'])){
				...
				if(!empty($user_token)){
					...
					$param_token = md5(md5($user_token.'float').$_SESSION['clientkey']);
					$p_token = I('param_token');
					if ($param_token != $p_token){
						$this->error('玩家未登陆,或登录失效!');
					}
					$_SESSION['username'] = M('members')->where(array('id'=>$_SESSION['mem_id']))->getfield('username');
