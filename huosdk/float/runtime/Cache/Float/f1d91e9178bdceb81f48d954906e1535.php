...
<div class="mode">
		<?php if(!empty($userdata['mobile']) OR !empty($userdata['email'])): ?><div>已绑定方式</div>
        <div class="bandWay">
        	<?php if(!empty($userdata['mobile'])): ?><div class="sj">
            ...
            <?php if(!empty($userdata['email'])): ?><div class="yx">
...
<?php else: ?>
<div>该账号未绑定,请联系客服</div><?php endif; ?>
        ...
        divs[i].onclick=function(){
            if(this.className=="sj"){
                if(document.querySelectorAll(".bandWay>div:first-child>div:first-child>span")[0].innerHTML){
                    window.location.href="<?php echo U('Forgetpwd/mobile_verify');?>";
                }else{
                	window.location.href="<?php echo U('Forgetpwd/mobile_verify');?>";
                }
                ...
                if(document.querySelectorAll(".bandWay>div:nth-child(2)>div:first-child>span")[0].innerHTML){
                    window.location.href="<?php echo U('Forgetpwd/email_verify');?>";
                }else{
                    window.location.href="<?php echo U('Forgetpwd/email_verify');?>";
                }
            }
